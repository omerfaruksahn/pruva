...
window.pricingCustomersView = (state) => {
    ...
    const getSensitivityBadge = (sens) => {
        switch(sens) {
            case 'HIGH': return '<span class="sens-badge high" style="background: rgba(239, 68, 68, 0.15); color: #ef4444; font-size: 0.72rem; padding: 4px 8px; border-radius: 4px; font-weight: 700;">Aşırı Duyarlı</span>';
            case 'LOW': return '<span class="sens-badge low" style="background: rgba(16, 185, 129, 0.15); color: #10b981; font-size: 0.72rem; padding: 4px 8px; border-radius: 4px; font-weight: 700;">Düşük Duyarlılık</span>';
            default: return '<span class="sens-badge normal" style="background: rgba(59, 130, 246, 0.15); color: #3b82f6; font-size: 0.72rem; padding: 4px 8px; border-radius: 4px; font-weight: 700;">Normal Duyarlılık</span>';
        }
    };

    const getTypeBadge = (type) => {
        if (type === 'VIP') {
            return '<span class="type-badge vip" style="background: linear-gradient(135deg, #f1c40f, #f39c12); color: white; font-size: 0.7rem; padding: 3px 8px; border-radius: 99px; font-weight: 800; box-shadow: 0 2px 6px rgba(241,196,15,0.3);">★ VIP ALICI</span>';
        }
        if (type === 'SENSITIVE') {
            return '<span class="type-badge sensitive" style="background: #e67e22; color: white; font-size: 0.7rem; padding: 3px 8px; border-radius: 99px; font-weight: 800;">FIRSATÇI</span>';
        }
        return '<span class="type-badge standard" style="background: #34495e; color: white; font-size: 0.7rem; padding: 3px 8px; border-radius: 99px; font-weight: 800;">STANDART</span>';
    };
    ...
    <div class="pricing-actions-page-container">
        ...
        <div class="pruva-ai-hero-header">
            <div class="hero-left-meta">
                ...
                <h2>Müşteri Portföyü ve Navlun Hassasiyet Paneli</h2>
                <p>Teklif fiyatlandırmalarında Pricing AI tarafından otomatik olarak VIP indirim veya fırsatçı marjların uygulanması için müşteri hassasiyet düzeylerini yönetin.</p>
            </div>
        ...
        <div class="actions-summary-grid" style="margin-bottom: 20px;">
            <div class="summary-card blue">
                ...
                <div class="summary-card-details">
                    ...
                    <span class="summary-card-label">Kayıtlı Müşteri</span>
                </div>
            </div>
            <div class="summary-card green">
                <div class="summary-card-icon">📊</div>
                <div class="summary-card-details">
                    <span class="summary-card-value">${avgVolume} FCL</span>
                    <span class="summary-card-label">Ort. Aylık Hacim</span>
                </div>
            </div>
            <div class="summary-card yellow">
                ...
                <div class="summary-card-details">
                    ...
                    <span class="summary-card-label">Yüksek Fiyat Hassasiyeti</span>
                </div>
            </div>
            <div class="summary-card purple" style="background: rgba(139, 92, 246, 0.05); border-color: rgba(139, 92, 246, 0.2);">
                ...
                <div class="summary-card-details">
                    ...
                    <span class="summary-card-label">VIP Profil Sayısı</span>
                </div>
            </div>
        </div>
            ...
            <aside class="ai-side-panel left-side" style="padding: 20px; background: var(--bg-surface); border: 1px solid var(--border); border-radius: var(--radius-lg); height: fit-content;">
                ...
                <form id="customer-profile-form" onsubmit="event.preventDefault(); window.pricingCustomersViewInit.saveCustomer();" style="display: flex; flex-direction: column; gap: 12px;">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="text-input" id="cust-form-name" placeholder="Örn: Arçelik A.Ş." required style="background: var(--bg-body); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 8px 12px; color: var(--text-primary);">
                    </div>
                    
                    <div class="input-group">
                        <label class="input-label" style="font-size: 0.7rem;">E-POSTA ADRESİ</label>
                        <input type="email" class="text-input" id="cust-form-email" placeholder="import@sirket.com" required style="background: var(--bg-body); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 8px 12px; color: var(--text-primary);">
                    </div>

                    <div class="input-group">
                        <label class="input-label" style="font-size: 0.7rem;">AYLIK LOJİSTİK HACİM (FCL/CBM)</label>
                        <input type="number" class="text-input" id="cust-form-volume" placeholder="Örn: 85" style="background: var(--bg-body); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 8px 12px; color: var(--text-primary);">
                    </div>
                            ...
                            <div class="carrier-card" style="padding: 16px; border: 1px solid var(--border); border-radius: var(--radius-md); background: rgba(255,255,255,0.02); display: flex; flex-direction: column; justify-content: space-between; gap: 12px; transition: all 0.3s ease;">
                                ...
                                <div style="font-size: 0.75rem; color: var(--text-secondary); background: rgba(255,255,255,0.01); border: 1px solid var(--border); padding: 8px; border-radius: var(--radius-sm);">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                        <strong>Aylık Hacim:</strong>
                                        <span style="color: var(--text-primary); font-weight: 700;">${cust.monthly_volume || 0} FCL / Ay</span>
                                    </div>
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <strong>Bölgeler:</strong>
                                        <span style="color: var(--text-primary); font-weight: 600; text-align: right; max-width: 140px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${cust.active_regions ? cust.active_regions.join(', ') : ''}">${cust.active_regions ? cust.active_regions.join(', ') : 'Ortak'}</span>
                                    </div>
                                </div>
...
window.pricingCustomersViewInit = async (app) => {
    ...
    window.pricingCustomersViewInit.saveCustomer = async () => {
        ...
        if (!name || !email) {
            showToast('Lütfen firma adı ve e-posta alanlarını doldurun.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                showToast('Müşteri profili başarıyla oluşturuldu ve eşitlendi!', 'success');
            ...
            showToast('Müşteri profili oluşturuldu (Yerel mod).', 'success');
            document.getElementById('customer-profile-form').reset();
        }
    };
    ...
    window.pricingCustomersViewInit.deleteCustomer = async (id) => {
        if (!confirm('Bu müşteri profilini silmek istediğinize emin misiniz?')) return;
        ...
        try {
            ...
            if (res.ok) {
                ...
                app.state._pricingCustomersLastFetched = Date.now();
                app.commit();
                showToast('Müşteri profili silindi.', 'danger');
            } else {
                throw new Error('API Hatası');
            }
        } catch (err) {
            ...
            app.state.pricingCustomers = app.state.pricingCustomers.filter(c => c.id !== id);
            app.commit();
            showToast('Müşteri profili silindi (Yerel mod).', 'danger');
        }
    };
};
