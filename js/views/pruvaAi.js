...
window.pruvaAiView = (state) => {
    ...
    const getStatusBadge = (status) => {
        switch(status) {
            case 'PENDING':
            case 'RATES_REQUESTED':
                return '<span class="status-badge" style="background: rgba(239,68,68,0.15); color: #ef4444; font-size: 0.7rem; font-weight: 700; padding: 3px 8px; border-radius: 4px;" data-i18n="pruva_ai.status_pending">Aksiyon Bekliyor</span>';
            case 'MISSING_INFO_SENT':
                return '<span class="status-badge" style="background: rgba(245,158,11,0.15); color: #f59e0b; font-size: 0.7rem; font-weight: 700; padding: 3px 8px; border-radius: 4px;" data-i18n="pruva_ai.status_missing_info">Bilgi Bekleniyor</span>';
            case 'COMPLETED':
            case 'OFFER_SENT':
                return '<span class="status-badge" style="background: rgba(16,185,129,0.15); color: #10b981; font-size: 0.7rem; font-weight: 700; padding: 3px 8px; border-radius: 4px;" data-i18n="pruva_ai.status_completed">Tamamlandı</span>';
            default:
                return '<span class="status-badge" style="background: rgba(156,163,175,0.15); color: #9ca3af; font-size: 0.7rem; font-weight: 700; padding: 3px 8px; border-radius: 4px;" data-i18n="pruva_ai.status_passive">Pasif</span>';
        }
    };
    ...
    <div class="pruva-ai-page-container pruva-saas-layout">
                ...
                ${state.outlookConnected ? `
                    <div style="display: flex; align-items: center; gap: 8px; background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.25); border-radius: var(--radius-md); padding: 8px 16px;">
                        <span style="color: #10b981; font-weight: 700; font-size: 0.8rem; display: flex; align-items: center; gap: 6px;">
                            🟢 <span data-i18n="pruva_ai.outlook_connected">Outlook Bağlı:</span> <strong style="color: var(--text-primary); font-weight: 700;">${state.outlookEmail || ''}</strong>
                        </span>
                        <button onclick="window.app.managers.pruvaAi.disconnectOutlook()" style="background: transparent; border: none; color: #ef4444; font-size: 0.95rem; font-weight: 700; cursor: pointer; padding: 0 4px; display: flex; align-items: center; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.15)'" onmouseout="this.style.transform='scale(1)'" data-i18n="[title]pruva_ai.disconnect" title="Bağlantıyı Kes">✕</button>
                    </div>
                ` : `
            ...
            <div class="chat-left-sidebar">
                ...
                <div style="display: flex; gap: 8px; margin: 12px 16px; align-items: center;">
                    <div class="chat-search-wrapper" style="flex: 1; margin: 0;">
                        <input type="text" class="chat-search-input" id="conv-search-input" data-i18n="[placeholder]pruva_ai.search_placeholder" placeholder="Konuşma veya içerik ara..." value="${state.convSearchQuery || ''}" oninput="window.pruvaAiManager.searchConversations(this.value)" style="margin: 0; width: 100%;">
                    </div>
                    <button class="btn btn-secondary" onclick="window.app.managers.pruvaAi.triggerMailScan()" data-i18n="[title]pruva_ai.scan_mail" title="Mail Tara" style="padding: 0; width: 36px; height: 36px; border-radius: var(--radius-md); cursor: pointer; background: var(--bg-elevated); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.2s;" onmouseover="this.style.background='var(--bg-surface)'; this.style.borderColor='var(--primary)'" onmouseout="this.style.background='var(--bg-elevated)'; this.style.borderColor='var(--border)'">
                        📥
                    </button>
                </div>
                ...
                <div class="chat-filter-tabs" style="padding: 0 16px 12px 16px; margin: 0; border-bottom: 1px solid var(--border);">
                    <select class="chat-filter-select" onchange="window.pruvaAiManager.filterConversations(this.value)" style="width: 100%; padding: 8px 12px; border-radius: var(--radius-md); border: 1px solid var(--border); background: var(--bg-surface); color: var(--text-primary); font-size: 0.85rem; font-weight: 600; cursor: pointer; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='var(--border)'">
                        <option value="ALL" ${filterMode === 'ALL' ? 'selected' : ''} data-i18n="pruva_ai.filter_all">📋 Tüm Konuşmalar</option>
                        <option value="CUSTOMERS" ${filterMode === 'CUSTOMERS' ? 'selected' : ''} data-i18n="pruva_ai.filter_customers">🏢 Sadece Müşteriler</option>
                        <option value="CARRIERS" ${filterMode === 'CARRIERS' ? 'selected' : ''} data-i18n="pruva_ai.filter_carriers">🚚 Sadece Taşıyıcılar</option>
                        <option value="PENDING" ${filterMode === 'PENDING' ? 'selected' : ''} data-i18n="pruva_ai.filter_pending">⏳ İşlem Bekleyenler</option>
                        <option value="ARCHIVED" ${filterMode === 'ARCHIVED' ? 'selected' : ''} data-i18n="pruva_ai.filter_archived">📦 Arşivlenenler</option>
                    </select>
                </div>
                    ...
                    <div class="chat-list-item ${activeConvId === 'copilot' ? 'active' : ''}" onclick="window.pruvaAiManager.selectConversation('copilot')" style="background: ${activeConvId === 'copilot' ? 'rgba(37,99,235,0.08)' : 'transparent'}; border-left: 3px solid #2563eb; margin-bottom: 8px; border-radius: var(--radius-md);">
                        ...
                        <div class="chat-item-details">
                            <div class="chat-item-row1">
                                <span class="chat-item-name" style="font-weight: 800; color: var(--primary);" data-i18n="pruva_ai.copilot_center">P-AI Komut Merkezi</span>
                            ...
                            <div class="chat-item-row2">
                                <span class="chat-item-snippet" style="color: var(--text-secondary); font-weight: 600;" data-i18n="pruva_ai.general_command">Genel Komut & AI Sohbeti</span>
            ...
            <div class="chat-right-panel">
                ${!activeConv ? `
                    ...
                    <div class="chat-welcome-box">
                        ...
                        <p style="font-size: 0.82rem; color: var(--text-secondary); max-width: 320px; line-height: 1.5; margin: 0 auto 20px;" data-i18n="pruva_ai.welcome_desc">Bir konuşma seçerek timeline akışını görüntüleyin veya yeni bir yapay zeka komutu gönderin.</p>
                    </div>
                ` : `
                    <!-- 2. KONUŞMA SEÇİLİNCE -->
                    <div class="chat-active-header">
                        <div class="chat-header-info">
                            ...
                            <div>
                                <h4 style="margin: 0; font-size: 0.9rem; font-weight: 700; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 300px;" title="${activeConv.id === 'copilot' ? 'P-AI Komut Merkezi' : (activeConv.subject || activeConv.company)}">${activeConv.id === 'copilot' ? '<span data-i18n="pruva_ai.copilot_center">P-AI Komut Merkezi</span>' : (activeConv.subject || activeConv.company)}</h4>
                                <div style="display: flex; align-items: center; gap: 6px; margin-top: 2px;">
                                    ${activeConv.id === 'copilot' ? `
                                        <div style="display: flex; align-items: center; gap: 4px;">
                                            ...
                                            <span style="font-size: 0.75rem; font-weight: 600; color: var(--text-secondary); opacity: 0.7;" data-i18n="${state.outlookConnected ? 'pruva_ai.ready' : 'pruva_ai.not_connected'}">${state.outlookConnected ? 'Hazır' : 'Bağlı Değil'}</span>
                                        </div>
                                ...
                                return `
                                    <div class="chat-bubble-row incoming">
                                        <div class="chat-bubble" style="position: relative; padding-right: 56px;">
                                            ...
                                            <!-- Speak Button -->
                                            <button onclick="window.app.managers.pruvaAi.speakText(this)" data-text="${escapeHTML(msg.text)}" style="position: absolute; top: 6px; right: 28px; background: transparent; border: none; cursor: pointer; opacity: 0.15; font-size: 1rem; transition: opacity 0.2s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.15" data-i18n="[title]pruva_ai.read_aloud" title="Sesli Oku">
                                ...
                                return `
                                    <div class="chat-bubble-row outgoing">
                                        <div class="chat-bubble" style="position: relative; padding-right: 32px;">
                                            ${escapeHTML(msg.text)}
                                            <span class="chat-bubble-meta">${msg.time}</span>
                                            <button onclick="window.app.managers.pruvaAi.copyText(this)" data-text="${escapeHTML(msg.text)}" style="position: absolute; top: 6px; right: 6px; background: transparent; border: none; cursor: pointer; color: white; opacity: 0.25; font-size: 1rem; transition: opacity 0.2s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.25" data-i18n="[title]pruva_ai.copy" title="Kopyala">
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="chat-bubble-row center">
                                        <div class="ai-suggestion-card" id="suggestion-card-${activeConv.id}-${index}" style="width: 100%; max-width: 500px;">
                                            <div class="ai-suggestion-title">
                                                <img src="/assets/pruva_robot.svg" style="width: 18px; height: 18px; vertical-align: text-bottom; margin-right: 4px;"> <span data-i18n="pruva_ai.ai_suggestion">Yapay Zeka Önerisi</span>
                                            </div>
                                            ...
                                            <div class="ai-suggestion-actions">
                                                <button class="btn btn-danger" onclick="window.pruvaAiManager.rejectSuggestion('${activeConv.id}', ${index})" style="padding: 4px 10px; font-size: 0.7rem; border-radius: var(--radius-sm);" data-i18n="pruva_ai.reject">Reddet</button>
                                                <button class="btn btn-primary" onclick="window.pruvaAiManager.approveSuggestion('${activeConv.id}', ${index}, '${msg.action || ''}')" style="padding: 4px 10px; font-size: 0.7rem; background: #d97706; border-color: #d97706; color: white; border-radius: var(--radius-sm);" data-i18n="pruva_ai.approve">Onayla</button>
                                            </div>
                                        </div>
                                    </div>
                                `;
        ...
        <div class="chat-details-drawer ${state.detailsDrawerOpen ? 'active' : ''}" id="details-drawer">
            <div class="drawer-header">
                <h3 data-i18n="pruva_ai.conversation_details">📋 Konuşma Detayları</h3>
            ...
            <div class="drawer-body" id="drawer-body-content">
                ${activeConv ? `
                    ...
                    <div class="drawer-section">
                        <div class="drawer-company-card">
                            ...
                            <div class="drawer-company-info">
                                ...
                                <p data-i18n="${activeConv.email ? '' : 'pruva_ai.no_email'}">${activeConv.email || 'Email bilgisi yok'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Durum Geçmişi -->
                    <div class="drawer-section">
                        <div class="drawer-section-title" data-i18n="pruva_ai.recent_activities">Son Aktiviteler</div>
                    ...
                    <!-- Notlar -->
                    <div class="drawer-section">
                        <div class="drawer-section-title" data-i18n="pruva_ai.notes">Notlar</div>
                        <textarea class="drawer-notes-area" id="conv-notes" data-i18n="[placeholder]pruva_ai.notes_placeholder" placeholder="Bu konuşma hakkında notlarınız...">${activeConv.notes || ''}</textarea>
                        <button class="drawer-notes-save" onclick="window.app.managers.pruvaAi.saveConversationNotes()" data-i18n="pruva_ai.save_note">💾 Notu Kaydet</button>
                    </div>
                ` : '<p style="color:var(--text-muted);text-align:center;margin-top:40px;" data-i18n="pruva_ai.no_conversation_selected">Konuşma seçilmedi</p>'}
            </div>
        </div>
